...
struct main_state {
	...
	/* Flags to track if each module with an unresponsive initial state is ready */
	struct {
		bool fota_ready;
		bool power_ready;
		bool location_ready;
...
/* Check if all modules that have an unresponsive initial state have reported ready.
 * If so, publish MAIN_MODULES_READY message to transition out of the waiting for modules state.
 */
static void check_modules_ready(struct main_state *state_object)
{
	if (state_object->modules_ready.fota_ready &&
	    state_object->modules_ready.power_ready &&
	    state_object->modules_ready.location_ready) {
		enum priv_main_msg_type msg = MAIN_MODULES_READY;
		int err = zbus_chan_pub(&priv_main_chan, &msg, PUB_TIMEOUT);
		if (err) {
...
static enum smf_state_result waiting_for_modules_init_run(void *o)
{
	...
	/* Update the extended state with any incoming ready message, and check if all modules are ready.
	 */
	if (state_object->chan == &fota_chan) {
		if (MSG_TO_FOTA_TYPE(state_object->msg_buf) == FOTA_READY) {
	 ...
	 */
		...
		if (MSG_TO_PRIV_MAIN_TYPE(state_object->msg_buf) == MAIN_MODULES_READY) {
...
static void disconnected_entry(void *o)
{
	...
	/* Once all modules are ready and the main module has started, trigger network connection */
	struct network_msg network_msg = {
		.type = NETWORK_CONNECT
	};

	int err = zbus_chan_pub(&network_chan, &network_msg, PUB_TIMEOUT);
	if (err) {
