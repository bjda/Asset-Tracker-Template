...
In the Asset Tracker Template, Each module declares and defines a channel using zbus macros. For example, in the `modules/network/network.h` file, the Network module declares the `NETWORK_CHAN` channel:
...
The channel is defined in `modules/network/network.c`:
...
The message type forms the first part of the message structure. For some message types, associated data is present in the message. When the data varies based on the message type, a union is used to save memory. In this case the message type determines which field is valid.
